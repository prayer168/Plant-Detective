...
- 結果：7 個 HTML 頁面、61 張校園植物卡、30 張芳香植物卡、6 張芳香植物圖版、6 張果實種子分類圖、8 套五感遊戲、42 張 IMAGE 2.0 PNG 資產。
- 內部 `href`／`src` 檔案存在性：通過。
- 占位文字掃描：通過，未發現 `待補`、`placeholder` 或 `lorem ipsum`。
- HTML 語系、教材頁籤導覽標籤：7 頁皆通過。
- Git：已初始化 `main` 分支；提交前 `git diff --cached --check` 通過。

## 瀏覽器與響應式驗證

測試頁面：`index.html`、`plants.html`、`seeds.html`、`leaves.html`、`fragrant-plants.html`、`game-one.html`、`game-two.html`。

| Viewport | 頁面數 | HTTP 200 | 主控台／頁面錯誤 | 圖片逐頁捲動後載入 | 水平溢出 |
|---|---:|---:|---:|---:|---:|
| 1440×1000 | 7 | 7/7 | 0 | 7/7 | 0 |
| 768×1024 | 7 | 7/7 | 0 | 7/7 | 0 |
| 390×844 | 7 | 7/7 | 0 | 7/7 | 0 |

初次檢查發現瀏覽器自動請求 favicon 造成一次 404；加入空資料 favicon 後，以相同 18 組頁面／viewport 完整重測，HTTP、console 與 page error 均為 0。

本次改版後以 `scripts/browser-verify.mjs` 再次完整重測；驗證器會強制載入並解碼所有圖片，再檢查 61 張校園植物卡、13 張校園植物圖版、30 張芳香植物卡、6 張芳香植物圖版、5 張葉子形態圖、6 張果實種子分類圖、8 套五感遊戲與 8 張步驟圖、標題層級及水平溢出。
...
- 全站 7 頁導覽列皆無圓形「葉」圖示；桌機與手機截圖確認只保留「植物偵探」文字首頁連結，導覽間距正常。
- 首頁封面已移除適用年級、教材型態與作答方式資訊條；桌機與手機截圖確認主標、副標及封面留白正常。
- 新增「會香的植物」頁：6 張五欄圖版逐張確認物種順序、樹形／株形、葉、花與果實辨識重點；桌機、平板、手機皆無裁切或重疊。頁面前 25 種為木本或木質化植物，後 5 種為草本，安全聞香提示在三種尺寸均清楚可讀。
- 圖版文字全部放在 HTML，不把生成圖內文字當作教材答案。

證據截圖位於 `docs/screenshots/`，每個頁面各保存 desktop、tablet、mobile 首屏，另保存植物頁、葉子形態頁與芳香植物頁尾端三種 viewport，共 30 張。

## 無障礙與列印

- 7 頁各有且只有 1 個 `h1`、1 個 `aria-current="page"`，無重複 ID。
- 所有內容圖片都有非空替代文字。
...
- 版本 1.3.1 推送至 `main` 後，GitHub Pages 狀態為 `built`。
...
- 公開頁面實測為 61 張不重複校園植物卡、13 張校園植物圖版、30 張芳香植物卡、6 張芳香植物圖版、5 張葉子形態圖、6 張果實種子分類圖、8 套五感遊戲與 8 張遊戲步驟圖。
- 公開站再次通過 7 頁 × 3 viewport 共 21 組 Playwright 測試，無主控台錯誤、破圖或水平溢出。
